...
export default function Tv_Search_Video({navigation}) {
  ...
  const fetchItemData = async search => {
    ...
    try {
      const response = await fetch(
        base_url + `tvProgmax/searchByTitle?query=${search}`,
        {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          base_url + `tvProgmax/searchByTitle?query=${selectedItemId}`,
          {
            method: 'GET',
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const loadSearchesFromStorage = async () => {
    try {
      const savedSearches = await AsyncStorage.getItem('tvProgmax');
      if (savedSearches) {
