...
//   export default function Fans_Search_Video({ navigation}) {
...
//           base_url + `fanStar/searchByTitle?query=${search}`,
//           {
//             method: "GET",
//             headers: {
...
//           const token = authToken;
//           const response = await fetch(
//             base_url + `fanStar/searchByTitle?query=${selectedItemId}`,
//             {
//               method: "GET",
//               headers: {
//                 Authorization: `Bearer ${token}`,
...
//         app.title && app.title.toLowerCase().includes(searchTerm)
//       );
...
//       try {
//         const savedSearches = await AsyncStorage.getItem("fansearchesProducts");
//         if (savedSearches) {
...
const loadSearchesFromStorage = async () => {
  try {
    const savedSearches = await AsyncStorage.getItem('fanStar');
    if (savedSearches) {
      setSearches(JSON.parse(savedSearches));
      getUserID();
    }
...
const renderAvailableApps = item => {
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() =>
        navigation.navigate("Fans_star_details", { videoData: item })}
