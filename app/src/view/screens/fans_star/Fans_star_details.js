...
export default function Fans_star_details({navigation, route}) {
  ...
  const fetchComments = async result => {
    ...
    try {
      const response = await fetch(
        base_url + `fanStar/getComments/${receivedData.video_id}?page=1&limit=100000`,
        {
          method: 'GET',
          headers: {
  ...
  const sendLikes = async () => {
    ...
    try {
        ...
        const response = await axios.post(
            base_url + 'fanStar/toggleLikeVideo',
            commentData,
            axiosConfig
        );
  ...
  const handleUpdateDelete = async () => {
    ...
    try {
      const response = await fetch(
        base_url + `fanStar/delete/${receivedData?.video_id}`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
