...
export default function Learning_details({ navigation, route }) {
  ...
  const fetchComments = async (result) => {
    ...
    try {
      const response = await fetch(
        base_url +
          `learningHobbies/getComments/${receivedData.video_id}?page=1&limit=100000`,
        {
          method: "GET",
          headers: {
  ...
  const sendComment = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        base_url + "learningHobbies/addComment",
        commentData,
        axiosConfig
      );
  ...
  const handleUpdateDelete = async () => {
    ...
    try {
      const response = await fetch(
        base_url + `learningHobbies/delete/${receivedData?.video_id}`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
