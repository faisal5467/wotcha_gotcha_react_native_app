...
export default function SearchScreenPicTours({navigation}) {
  ...
  const fetchItemData = async search => {
    ...
    try {
      const response = await fetch(
        base_url + `picTour/searchTour?name=${search}`,
        {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          base_url + `picTour/searchTour?name=${selectedItemId}`,
          {
            method: 'GET',
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const saveSearchTerm = async () => {
    ...
    try {
      ...
      await AsyncStorage.setItem('searchPics', JSON.stringify(updatedSearches));
